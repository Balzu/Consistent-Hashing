package it.unipi.mcsn.pad.consistent;
...
public class ConsistentHasherImpl<B, M> implements ConsistentHasher<B, M> 
{
	...
	private boolean removeBucket(B bucketName, long timeout, TimeUnit unit,	boolean tryLock) throws InterruptedException 
	{
		...
		if (bucketInfo == null) //Don't have a bucket with that name: nothing more to do
			return true;
	...
	@Override
	public B findBucket(M member)
	{
		Preconditions.checkNotNull(member, "Member can not be null.");
		ByteBuffer byteMember = convertAndApplyHash(member);
		//Find the bucket for the provided member, i.e. the next node clockwise in the ring
		Entry<ByteBuffer, B> targetBucketentry = bucketsMap.ceilingEntry(byteMember);		
		//If there is no bucket greater than the provided member, than the next bucket
		// clockwise in the ring is the first (the smallest) bucket
		return targetBucketentry!= null ? targetBucketentry.getValue()
				: bucketsMap.firstEntry().getValue();
	...
	private static <T> Optional<T> getLastKey(NavigableMap<T, ?> map) 
	{
		...
		Optional<T> result = Optional.fromNullable(key);
		return result;
	}
}
